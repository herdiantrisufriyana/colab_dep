...
author:
  ...
  - name: Herdiantri Sufriyana, MD, PhD
    affiliation:
...
ggsave('fig1.tiff',width=179,height=179/190*95,units='mm',dpi=1000)
```
...
vimp_spc_gbm %>%
  ...
  mutate(
    weight=
      case_when(
        weight<(floor((min(weight)+1*(max(weight)-min(weight))/4)*100)/100)
        ~paste0('<',(floor((min(weight)+1*(max(weight)-min(weight))/4)*100)/100))
        ,weight>=(floor((min(weight)+(max(weight)-min(weight))/4)*100)/100)
         & weight<(floor((min(weight)+2*(max(weight)-min(weight))/4)*100)/100)
         ~paste0(
           (floor((min(weight)+1*(max(weight)-min(weight))/4)*100)/100)
           ,' to '
           ,'<',(floor((min(weight)+2*(max(weight)-min(weight))/4)*100)/100)
          )
        ,weight>=(floor((min(weight)+2*(max(weight)-min(weight))/4)*100)/100)
         & weight<=(floor((min(weight)+3*(max(weight)-min(weight))/4)*100)/100)
         ~paste0(
           (floor((min(weight)+2*(max(weight)-min(weight))/4)*100)/100)
           ,' to '
           ,(floor((min(weight)+3*(max(weight)-min(weight))/4)*100)/100)
          )
        ,TRUE
         ~paste0('>',(floor((min(weight)+3*(max(weight)-min(weight))/4)*100)/100))
      )
  ) %>%
  mutate(
    weight=
      weight %>%
      factor(c('<0.07','0.07 to <0.15','0.15 to 0.23','>0.23'))
  ) %>%
  ...
  #   ,midpoint=0
  # ) +
  scale_fill_manual('Weight',values=c('grey10','grey33','grey57','grey80'))+
  theme(
    text=element_text(family='sans',size=8)
...
ggsave('fig2.tiff',width=179,height=179/140*210,units='mm',dpi=500)
```
...
set$divnn$viz.ontonet=
  ...
  viz.ontonet(
    ...
    ,eval.metric='roc'
    ,eval.pal=c('#E64B35FF','#4DBBD5FF')#'#00A087FF'
  )
```
...
plot.viz.ontonet = function(x
  ...
  p +
    ...
    scale_color_gradient(low='#E64B35FF',high='#4DBBD5FF') + #'#00A087FF'
    scale_fill_gradient(low='#E64B35FF',high='#4DBBD5FF') + #'#00A087FF'
    theme_blank()
}
```
...
ggarrange(
    ...
    ,widths=c(3.5,3.5)
    ,heights=5
    ,font.label=list(family='sans',size=10)
  )

ggsave('fig3.tiff',width=179,height=179/140*100,units='mm',dpi=300)
```
